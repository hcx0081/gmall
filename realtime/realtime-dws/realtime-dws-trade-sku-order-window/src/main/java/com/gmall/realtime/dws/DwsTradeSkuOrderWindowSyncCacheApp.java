...
public class DwsTradeSkuOrderWindowSyncCacheApp extends BaseApp {
    ...
    public void handle(StreamExecutionEnvironment env, DataStreamSource<String> source) {
        ...
        SingleOutputStreamOperator<TradeSkuOrder> fullDimStream = reducedStream.map(new RichMapFunction<TradeSkuOrder, TradeSkuOrder>() {
            ...
            public TradeSkuOrder map(TradeSkuOrder value) throws Exception {
                ...
                String dimSkuInfoKey = getKey(Constants.HBASE_TABLE_DIM_SKU_INFO, value.getSkuId());
                String dimSkuInfoStr = jedis.get(dimSkuInfoKey);
                if (StringUtils.isBlank(dimSkuInfoStr)) {
                    dimSkuInfo = HBaseUtils.get(connection, Constants.HBASE_NAMESPACE, Constants.HBASE_TABLE_DIM_SKU_INFO, value.getSkuId(), Constants.HBASE_COLUMNFAMILY);
                    if (MapUtils.isNotEmpty(dimSkuInfo)) {
                        jedis.set(dimSkuInfoKey, JSON.toJSONString(dimSkuInfo), SetParams.setParams().ex(60 * 60 * 24));
                    }
                } else {
                    dimSkuInfo = JSON.parseObject(dimSkuInfoStr);
                }
                if (MapUtils.isEmpty(dimSkuInfo)) {
                    log.error("没有对应的维度信息: {}", dimSkuInfo);
                } else {
                ...
                JSONObject dimSpuInfo;
                String dimSpuInfoKey = getKey(Constants.HBASE_TABLE_DIM_SPU_INFO, value.getSpuId());
                String dimSpuInfoStr = jedis.get(dimSpuInfoKey);
                if (StringUtils.isBlank(dimSpuInfoStr)) {
                    dimSpuInfo = HBaseUtils.get(connection, Constants.HBASE_NAMESPACE, Constants.HBASE_TABLE_DIM_SPU_INFO, value.getSpuId(), Constants.HBASE_COLUMNFAMILY);
                    if (MapUtils.isNotEmpty(dimSpuInfo)) {
                        jedis.set(dimSpuInfoKey, JSON.toJSONString(dimSpuInfo), SetParams.setParams().ex(60 * 60 * 24));
                    }
                } else {
                    dimSpuInfo = JSON.parseObject(dimSpuInfoStr);
                }
                if (MapUtils.isEmpty(dimSpuInfo)) {
                    log.error("没有对应的维度信息: {}", dimSpuInfo);
                } else {
                    value.setSpuName(dimSpuInfo.getString("spu_name"));
                }
                
                JSONObject dimBaseCategory3;
                String dimBaseCategory3Key = getKey(Constants.HBASE_TABLE_DIM_BASE_CATEGORY3, value.getCategory3Id());
                String dimBaseCategory3Str = jedis.get(dimBaseCategory3Key);
                if (StringUtils.isBlank(dimBaseCategory3Str)) {
                    dimBaseCategory3 = HBaseUtils.get(connection, Constants.HBASE_NAMESPACE, Constants.HBASE_TABLE_DIM_BASE_CATEGORY3, value.getCategory3Id(), Constants.HBASE_COLUMNFAMILY);
                    if (MapUtils.isNotEmpty(dimBaseCategory3)) {
                        jedis.set(dimBaseCategory3Key, JSON.toJSONString(dimBaseCategory3), SetParams.setParams().ex(60 * 60 * 24));
                    }
                } else {
                    dimBaseCategory3 = JSON.parseObject(dimBaseCategory3Str);
                }
                if (MapUtils.isEmpty(dimBaseCategory3)) {
                    log.error("没有对应的维度信息: {}", dimBaseCategory3);
                } else {
                    value.setCategory2Id(dimBaseCategory3.getString("category2_id"));
                    value.setCategory3Name(dimBaseCategory3.getString("name"));
                }
                
                JSONObject dimBaseCategory2;
                String dimBaseCategory2Key = getKey(Constants.HBASE_TABLE_DIM_BASE_CATEGORY2, value.getCategory2Id());
                String dimBaseCategory2Str = jedis.get(dimBaseCategory2Key);
                if (StringUtils.isBlank(dimBaseCategory2Str)) {
                    dimBaseCategory2 = HBaseUtils.get(connection, Constants.HBASE_NAMESPACE, Constants.HBASE_TABLE_DIM_BASE_CATEGORY2, value.getCategory2Id(), Constants.HBASE_COLUMNFAMILY);
                    if (MapUtils.isNotEmpty(dimBaseCategory2)) {
                        jedis.set(dimBaseCategory2Key, JSON.toJSONString(dimBaseCategory2), SetParams.setParams().ex(60 * 60 * 24));
                    }
                } else {
                    dimBaseCategory2 = JSON.parseObject(dimBaseCategory2Str);
                }
                if (MapUtils.isEmpty(dimBaseCategory2)) {
                    log.error("没有对应的维度信息: {}", dimBaseCategory2);
                } else {
                    value.setCategory1Id(dimBaseCategory2.getString("category1_id"));
                    value.setCategory2Name(dimBaseCategory2.getString("name"));
                }
                
                JSONObject dimBaseCategory1;
                String dimBaseCategory1Key = getKey(Constants.HBASE_TABLE_DIM_BASE_CATEGORY1, value.getCategory1Id());
                String dimBaseCategory1Str = jedis.get(dimBaseCategory1Key);
                if (StringUtils.isBlank(dimBaseCategory1Str)) {
                    dimBaseCategory1 = HBaseUtils.get(connection, Constants.HBASE_NAMESPACE, Constants.HBASE_TABLE_DIM_BASE_CATEGORY1, value.getCategory1Id(), Constants.HBASE_COLUMNFAMILY);
                    if (MapUtils.isNotEmpty(dimBaseCategory1)) {
                        jedis.set(dimBaseCategory1Key, JSON.toJSONString(dimBaseCategory1), SetParams.setParams().ex(60 * 60 * 24));
                    }
                } else {
                    dimBaseCategory1 = JSON.parseObject(dimBaseCategory1Str);
                }
                if (MapUtils.isEmpty(dimBaseCategory1)) {
                    log.error("没有对应的维度信息: {}", dimBaseCategory1);
                } else {
                    value.setCategory1Name(dimBaseCategory1.getString("name"));
                }
                
                JSONObject dimBaseTrademark;
                String dimBaseTrademarkKey = getKey(Constants.HBASE_TABLE_DIM_BASE_TRADEMARK, value.getTrademarkId());
                String dimBaseTrademarkStr = jedis.get(dimBaseTrademarkKey);
                if (StringUtils.isBlank(dimBaseTrademarkStr)) {
                    dimBaseTrademark = HBaseUtils.get(connection, Constants.HBASE_NAMESPACE, Constants.HBASE_TABLE_DIM_BASE_TRADEMARK, value.getTrademarkId(), Constants.HBASE_COLUMNFAMILY);
                    if (MapUtils.isNotEmpty(dimBaseTrademark)) {
                        jedis.set(dimBaseTrademarkKey, JSON.toJSONString(dimBaseTrademark), SetParams.setParams().ex(60 * 60 * 24));
                    }
                } else {
                    dimBaseTrademark = JSON.parseObject(dimBaseTrademarkStr);
                }
                if (MapUtils.isEmpty(dimBaseTrademark)) {
                    log.error("没有对应的维度信息: {}", dimBaseTrademark);
                } else {
    ...
    public String getKey(String hbaseTable, String id) {
        return Constants.HBASE_NAMESPACE + ":" + hbaseTable + ":" + id;
    }
}
